...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-eq-cream">
      ...
      {/* ── Page layout ── */}
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 flex gap-6 items-start">

        {/* ── Sidebar ── */}
        <aside className="hidden lg:flex flex-col items-center w-56 xl:w-64 bg-eq-navy rounded-2xl shadow-md py-8 px-5 sticky top-8 shrink-0">
          <img
            src={`${import.meta.env.BASE_URL}logo.png`}
            alt="Stable Terms"
          ...
          <p className="text-white/60 text-xs text-center leading-relaxed mb-2">
            {categories.reduce((sum, c) => sum + c.terms.length, 0)}+ terms
          </p>
          <p className="text-white/40 text-xs text-center italic">
            {categories.length} categories
          </p>
          <div className="mt-8 text-white/20 text-xs text-center">© Stable Terms</div>
        </aside>
